...
- **`~/.local/lib/myctl/`**: The directory containing all the logic.
  - **`_utils.sh`**: Contains essential helper functions (`import-lib`, `help-menu`, `log.*`, etc.).
...
5.  The `import-lib "script.sh"` function is called to `source` the necessary script file from the library.
...
2.  **Define the `help` subcommand:** Inside the `help` block, create a `case` for its own arguments. The first thing should always be the `help` option, which defines the `cmd_map` array for the `help-menu`.
3.  **Implement the command logic:** Add a case that calls your new function. Use `import-lib` to load your script first.
