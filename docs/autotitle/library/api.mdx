...
Disables metadata tagging for a specific operation, even if `mkvpropedit` or `atomicparsley` is available.
...
```go
// Skip tagging for this one call
ops, err := autotitle.Rename(ctx, ".", autotitle.WithNoTagging())
```
