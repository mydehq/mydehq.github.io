...
See the [API Reference: EventType](./api#eventtype) for a full list of event types and their meanings.
...
:::tip
You can see the CLI's handler implementation in [`internal/cli/root.go`](https://github.com/mydehq/autotitle/blob/main/internal/cli/root.go#L60) for a complete example using `id/charmbracelet/log`.
:::
...
// Generate filename from fields using a compiled pattern
vars := autotitle.TemplateVars{
    "SERIES": "Attack on Titan",
    "EP_NUM": "01",
    "EP_NAME": "To You, 2000 Years Later",
}
filename, err := autotitle.GenerateFilenameFromFields(pattern, vars)
```
